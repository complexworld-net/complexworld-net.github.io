...
to setup
  ...
  ask patches [
    set status 0
    set pcolor green
    set tau (min-tau + (max-tau - min-tau) * pxcor / world-width)
  ]
...
to go
  ...
  if not any? patches with [status = 2] [
    clear-plot
    set-plot-x-range min-tau max-tau
    let x 0
    while [x < max-pxcor] [
      plotxy  min-tau + (max-tau - min-tau) * x / world-width count patches with [status = 1 and pxcor = x]
      set x x + 1
    ]
    stop
  ]
