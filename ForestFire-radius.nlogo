...
to setup
  ...
  set max-neighbors 0
  ask patch 0 0 [
    set max-neighbors count patches in-radius radius - 1
  ]
  ask patches with [random-float 1 < tree-probability] [
    set pcolor green
    set status 1
    let n count patches in-radius radius
  ]
...
to go
  if not any? patches with [status = 3] [ stop ]
  ask patches with [status = 3] [
    set status 2
    set pcolor yellow
  ]
  ask patches with [status = 1][
    if any? patches in-radius radius with [status = 2] [ ; burning or burned is the same
      set status 3
      set pcolor red
    ]
  ]
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot (count patches with [status = 2] / initial-trees) * 100"
