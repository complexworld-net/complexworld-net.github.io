patches-own [density infectivity state]

globals [xx yy rr submitted?]
...
to go
  ...
  if not any? patches with [state = 2] [
    ask patches with [state = 4] [set pcolor grey]
    let sumq count patches with [state = 4]
...
to upload
  let args (list
    ...
    (list "entry.1621191553" (word precision radius 3))
