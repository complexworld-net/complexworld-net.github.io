...
to setup
  ...
  ask patches with [(random-float 1) < min-density + (max-density - min-density) * pxcor / world-width][
    set pcolor green
    set status 1
  ]
...
to go
  ...
  ask patches with [status = 1][
    if any? neighbors4 with [status = 2] [ ; burning or burned is the same
      set status 3
      set pcolor red
    ]
  ]
  if not any? patches with [status = 3] [
    clear-plot
    set-plot-x-range min-density max-density
    let x 0
    while [x < max-pxcor] [
      plotxy  min-density + (max-density - min-density) * x / world-width count patches with [status = 2 and pxcor = x]
      set x x + 1
    ]
    stop]
  tick
