...
to go
  if not any? patches with [status = 2] and beat = 0 [ stop ]
  ask patches with [status = 2] [
    ask neighbors4 with [status = 1] [
      set exposed? true
    ]
  ]
  ask patches with [status > 1][
    set status status + 1
    ifelse refractory-states >= 0 and status > refractory-states + 2 [
      set status 1
      set pcolor green
    ][
      set pcolor yellow
  ...
  ask patches with [exposed? = true] [
    set status 2
    set pcolor red
    set exposed? false
  ]
  if beat > 0 and ticks mod beat = 0 [
    full-line
  ]
  tick
end
...
to half-line
  ask patches with [pxcor = 0 and abs(pycor) < world-height / 4] [
    set status 2
    set pcolor red
  ]
  ask patches with [pxcor = -1 and abs(pycor) < world-height / 4] [
    set status 3
    set pcolor yellow
  ]
end
...
to full-line
  ask patches with [pxcor = min-pxcor + 10] [
    set status 2
    set pcolor red
  ]
  ask patches with [pxcor = min-pxcor + 9] [
    set status 3
    set pcolor yellow
  ]
end
